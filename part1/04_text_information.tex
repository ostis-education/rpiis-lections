...
\begin{frame}{\\Понятие текста}
	...
	\begin{SCn}
		...
		\begin{scnrelfromset}{структура}
			\scnfileitem{начало}
			\scnfileitem{конец}
			\scnfileitem{внутренняя структура}
		\end{scnrelfromset}

		\begin{scnrelfromlist}{свойство}
			\scnfileitem{линейность}
			\scnfileitem{дискретность}
		\end{scnrelfromlist}
...
\begin{frame}{\\Структура текста}
	...
	\begin{SCn}
		...
		\scntext{определение}{\textbf{\textit{структура текста}} -- это совокупность логических частей и элементов, из которых он состоит, и связей между ними.}		
...
\begin{frame}{\\Способы визуализации структуры документа}
	...
	\begin{SCn}
		...
		\begin{scnrelfromset}{способы визуализации}
			\scnfileitem{Размер шрифта}
			\scnfileitem{Отступ}
			\scnfileitem{Нумерация заголовков}
		\end{scnrelfromset}
	\end{SCn}
...
\begin{frame}{\\Удобство представления текста на компьютере}
	...
	\begin{SCn}
		...
		\begin{scnrelfromset}{требования}
			\scnfileitem{Простота кодирования}
			\scnfileitem{Точность представления}
			\scnfileitem{Удобство хранения}
			\scnfileitem{Удобство передачи по каналам связи}
		\end{scnrelfromset}
	\end{SCn}
\end{frame}
...
\begin{frame}{\\Терминология}
	...
	\begin{SCn}
		...
		\scnidtf{code page}
		\scntext{пояснение}{\textbf{\textit{Кодовая страница}} -- таблица, сопоставляющая каждому значению байта некоторый символ (или его отсутствие).}

		\scnheader{набор символов}
		\scnidtf{character set}
		\scntext{пояснение}{\textbf{\textit{Набор символов}} -- определённая таблица кодировки конечного множества знаков. Такая таблица сопоставляет каждому символу последовательность длиной в один или несколько байтов.}
		\begin{scnindent}
...
\begin{frame}{\\Проблемы однобайтного кодирования}
	...
	\begin{SCn}
		...
		\begin{scnrelfromset}{недостатки}
			\scnfileitem{проблема отображения документов в неправильной кодировке}
			\begin{scnindent}
				\scnidtf{"крокозябры"{}}
			\end{scnindent}
			\scnfileitem{проблема ограниченности набора символов}
			\scnfileitem{проблема преобразования одной кодировки в другую}
			\scnfileitem{проблема многоязычных документов}
			\scnfileitem{проблема дублирования шрифтов}
		\end{scnrelfromset}
	\end{SCn}
\end{frame}
...
\begin{frame}{\\Регулярные выражения}
	...
	\begin{SCn}
		\scnheader{регулярное выражение}
		\scnidtf{regular expression}
