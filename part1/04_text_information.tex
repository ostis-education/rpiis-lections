...
\begin{frame}{\\Структура лекции}
	...
	\begin{SCn}
		\scnheader{Лекция 7. Представление и обработка текстовой информации}
...
\begin{frame}{\\Понятие текста}
	...
	\begin{SCn}
		\scnheader{текст}
		\scntext{пояснение}{Текст - связная последовательность знаков, образующая законченное сообщение.}
...
\begin{frame}{\\Структура текста}
	...
	\begin{SCn}
		...
		\begin{scnrelfromlist}{пояснение}
			\scnfileitem{Структура – это совокупность логических частей и элементов, из которых он состоит, и связей между ними.}
			\scnfileitem{Структура текста зачастую определяется неявно, через \scnkeyword{форму}.}
			\begin{scnindent}
				\scntext{пример}{оформление задач по принципу: Дано-найти-решение.}
			\end{scnindent}
		\end{scnrelfromlist}
	\end{SCn}
...
\begin{frame}{\\Правила построения структуры текста}
	...
	\begin{SCn}
		...
		\begin{scnrelfromset}{правила построения}
			...
			\scnfileitem{Структура должны быть визуально подчеркнута.}
		\end{scnrelfromset}		
	\end{SCn}
\end{frame}
...
\begin{frame}{\\Терминология}
	...
	\begin{SCn}
		...
		\scnidtf{code page}
		\scntext{пояснение}{Кодовая страница -- таблица, сопоставляющая каждому значению байта некоторый символ (или его отсутствие).}

		\scnheader{набор символов}
		\scnidtf{character set}
		\scntext{пояснение}{Набор символов -- определённая таблица кодировки конечного множества знаков. Такая таблица сопоставляет каждому символу последовательность длиной в один или несколько байтов.}
		\begin{scnindent}
...
\begin{frame}{\\Регулярные выражения}
	...
	\begin{SCn}
		...
		\scntext{пояснение}{регулярные выражения -- способ (язык) описания множества строк}
...
\begin{frame}{\\Шаблон поиска}
	...
	\begin{SCn}
		...
		\scntext{пояснение}{шаблон поиска -- строка-описание желаемого результата поиска.}
		\scntext{пример}{a*.txt -- поиск всех текстовых файлов с именем на a}

		\scnheader{литерал}
		\scntext{пояснение}{Литерал – символ в шаблоне поиска, который соответствует самому себе.}

		\scnheader{метасимвол}
		\scntext{пояснение}{символ в шаблоне поиска, имеющий особое значение, обозначает какой-то другой символ или последовательность символов.}
	\end{SCn}
...
\begin{frame}{\\Символьные классы}
	...
	\begin{SCn}
		...
		\scntext{пояснение}{Символьный класс – любой символ из указанного набора символов}
	\end{SCn}
...
\begin{frame}{\\Сокращенная запись символьных классов}
	...
	\begin{SCn}
		\scnheader{cокращенная запись символьных классов}
...
\begin{frame}{\\Квантификаторы}
	...
	\begin{SCn}
		...
		\scntext{пояснение}{Квантификаторы -- метасимволы, указывающие сколько раз должен встретиться элемент перед ними.}

		\scnheader{жадное поведение}
		\scntext{пояснение}{Жадное поведение -- ищется строка максимальной длины, удовлетворяющая шаблону.}

		\scnheader{ленивое поведение}
		\scntext{пояснение}{Ленивое поведение -- ищется строка минимальной длины, удовлетворяющая шаблону.}
	\end{SCn}
...
\begin{frame}{\\Группы и подстановки}
	...
	\begin{SCn}
		\scnheader{группа}
		\scntext{пояснение}{Группа -- часть шаблона, которая обрабатывается как единое целое.}
		\scntext{пример}{(\textbackslash w+) (\textbackslash w+)\textbackslash. -- поиск имени и фамилии}

		\scnheader{подстановка}
		\scntext{пояснение}{Подстановка -- использование при замене найденных подстрок}
		\scntext{пример}{\textbackslash 2 \textbackslash 1\textbackslash. -- перемена местами имени и фамилии из примера выше.}
	\end{SCn}
...
\begin{frame}{\\Альтернативы}
	...
	\begin{SCn}
		...
		\scntext{пояснение}{Альтернатива -- выбор из нескольких вариантов}
